...
def fit_centered_2d_gaussian(racf):
    """
    Fit a Gaussian to the autocorrelation so that we can mask out the signal when calculating autocorrelation SNR.

    Args:
      racf: The autocorrelation 
    """

    # Determine grid coordinates centered at array center
    ny, nx = racf.shape
    x = np.arange(nx) - (nx - 1) // 2 ## meshgrid centered at zero
    y = np.arange(ny) - (ny - 1) // 2
    X, Y = np.meshgrid(x, y)

    def model(params): ## Gaussian model to fit to racf
      sx, sy, sxy = params
      # Build covariance matrix
      cov = np.array([[sx**2, sxy], [sxy, sy**2]])
      inv_cov = np.linalg.inv(cov)
      # Compute exponent for each (x,y)
      exponent = (
        inv_cov[0, 0] * X**2
        + 2 * inv_cov[0, 1] * X * Y
        + inv_cov[1, 1] * Y**2
      )
      return np.exp(-0.5 * exponent).ravel()

    def residuals(params): ## residual function
      return model(params) - racf.ravel()

    # Initial guess: isotropic width ~ 1/4 of image size, zero covariance
    initial_guess = [nx / 4, ny / 4, 0.0]
    # Assume Gaussian larger than zero and sigma smaller than window. Assume real covariance.
    bounds = ([1e-6, 1e-6, -abs(nx*ny)], [nx, ny, abs(nx*ny)])

    # Fit via scipy least squares. Could use another value if preferred.
    result = least_squares(residuals, initial_guess, bounds=bounds)
    sx_fit, sy_fit, sxy_fit = result.x
    fitted_gaussian = model(result.x).reshape(racf.shape)

    return fitted_gaussian

def measure_noise(racf):
    """
    Measure noise scale in the autocorrelation by fitting a Gaussian to the center, then measuring the remainder. 

    Args:
      racf: the autocorrelation 
    """

    fitted_gaussian = fit_centered_2d_gaussian(racf)

    y_remainder = racf - fitted_gaussian
    sigma_noise = np.std(y_remainder)

    return sigma_noise

def snr_xicrit_calibration(sigma_noise, turning_point=0.25, max_value=0.8):
    """
    Take a noise scale and returns a best xi_crit. As the noise scale increases, we require higher integration bounds to accurately measure 2nd moments. Optimum integration bounds threshold is roughly linear with noise up to a turning point, when noise dominates. The trning point and asymptotic value were optimized in test problems with Sgr A* April 11 duration, cadence, and with correlation length/time consistent with April 11 reconstructions and GRMHD.  

    Args:
      sigma_noise: the noise level
    """

    if sigma_noise < turning_point:
      return (max_value/turning_point) * sigma_noise
    else:
      return max_value
...
def calculate_pattern_speed(racf, dt, dtheta=2.0, xi_crit_factor=3.0):
    ...
    # Improve xi_crit ceiling check
    non_zero_columns = np.count_nonzero( np.sum(Q, axis=0) )
    non_zero_rows = np.count_nonzero( np.sum(Q, axis=1) )
    if (xi_crit > 1) or (non_zero_columns < 5) or (non_zero_rows < 5):
