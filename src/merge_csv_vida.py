...
for method_name, file_path in paths.items():
    ...
    # Keep the "time" column as is and rename other columns to include method name
    time_column = df["time"]
    df = df.drop(columns=["time"], errors="ignore")
    df = df.add_suffix(f"_{method_name}")
    df["time"] = time_column  # Add the time column back

    dfs.append(df)

# Concatenate along columns since all CSVs have the same number of rows
merged_df = pd.concat(dfs, axis=1)

# Ensure only one "time" column exists in the final DataFrame
