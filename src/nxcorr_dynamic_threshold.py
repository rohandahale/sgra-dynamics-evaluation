...
for pol in pollist:
    ...
    for p in polpaths.keys():
        ...
        for im, imt in zip(im_array, imt_array):
            if pol=='I':
                im.ivec= np.clip(np.array(im.imarr(pol='I')-median).flatten(), 1e-7, 1000)
                imt.ivec= np.clip(np.array(imt.imarr(pol='I')-median_t).flatten(), 1e-7, 1000)
            elif pol=='P' or pol=='X':
                im.ivec= np.clip(np.array(im.imarr(pol='I')-median).flatten(), 1e-7, 1000)
                imt.ivec= np.clip(np.array(imt.imarr(pol='I')-median_t).flatten(), 1e-7, 1000)
                im.qvec= np.array(im.imarr(pol='Q')-medianQ).flatten()
                imt.qvec= np.array(imt.imarr(pol='Q')-medianQ_t).flatten()
                im.uvec= np.array(im.imarr(pol='U')-medianU).flatten()
                imt.uvec= np.array(imt.imarr(pol='U')-medianU_t).flatten()
        ...
        for im, imt in zip(im_array, imt_array):
            ...
            if pol == 'I':
                ...
                nxcorr = imt2.compare_images(im2, pol='I', metric=['nxcorr'], shift=[0,0])
                nxcorr_t.append(nxcorr[0][0])
                nxcorr_tab.append(nxcorr[0][0])
        ...
        nxcorr_dyn[pol][p]=nxcorr_t
        nxcorr_dyn_pass[pol][p]=count/len(nxcorr_t)*100
    nxcorr_dyn_thres[pol]=w_ncri
...
# Iterate through nxcorr_dyn and add columns
for key_main, sub_dict in nxcorr_dyn.items():
    for key_sub, values in sub_dict.items():
        df[f"nxcorr_dyn_{key_main}_{key_sub}"] = values

# Add nxcorr_dyn_thres columns for each key_main (same values for all)
for key_main, values in nxcorr_dyn_thres.items():
    df[f"nxcorr_dyn_thres_{key_main}"] = values

# Iterate through nxcorr_dyn_pass and add columns
for key_main, sub_dict in nxcorr_dyn_pass.items():
    for key_sub, values in sub_dict.items():
        df[f"nxcorr_dyn_pass_{key_main}_{key_sub}"] = values
